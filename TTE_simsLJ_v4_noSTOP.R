...
for(eff.pattern in 1){
  for(eff in 1:4){
    for(tox in 1:5){
      assign(paste(c("comparison_comboLJv4noSTOP_cyc3_eff",eff,".",eff.pattern,"_tox",tox),collapse=""),
             foreach(i=1:nsims, combine = list) %dopar% {
               ...
               LiuJohnson_v2(seed=i,tru.E.pars = get(paste(c("eff_scen",eff,".",pattern,"pars"),collapse="")),
                             ...
                             sufficient.information=F,sufficient.information.lim=stop.num,hard.safety.rule=95,safety.stopping.low.unsafe=F,
                             safety.stopping.high.toosafe=F,kfold.skipping=T,kfold=2,initial.one.cycle=T,
      ...
      save.image(paste(c("Comparison_combo_cycles_TTE_LJnoSTOP_v4.RData"),collapse=""))
    }
  }
}
