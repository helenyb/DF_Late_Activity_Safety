#Joint TITE-CRM (with no-TITE version)
library(rjags)
...
CRM_2_combo_v12<-function(seed,tru.E.pars,tru.T.pars,tru.corET,co_size,ncohorts ,target,targetE,doses,p_a_meanT=0,p_b_meanT=0,
                          ...
                          backfill=F,backfill.num=2,TITE=T){
  set.seed(seed)
  stop_vec<-rep(0,7)
  ndoses<-length(doses)
  dosevec<-c()
