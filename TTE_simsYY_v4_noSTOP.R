...
for(eff.pattern in 1){
  for(eff in 1:4){
    for(tox in 1:5){
      assign(paste(c("comparison_combo2_cyc3YYv4noSTOP_eff",eff,".",eff.pattern,"_tox",tox),collapse=""),
             foreach(i=1:nsims, combine = list) %dopar% {
               ...
               Yuan2008_comp_v4(seed=i,tru.E.pars = get(paste(c("eff_scen",eff,".",pattern,"pars"),collapse="")),
                                ...
                                sufficient.information=F,sufficient.information.lim=29,hard.safety.rule=95,
                                safety.stopping.low.unsafe=F,
                                safety.stopping.high.toosafe=F,kfold.skipping=T,kfold=2,
                                precision.stopping=F,precision.stopping.level=29,initial.one.cycle=T,
      ...
      save.image(paste(c("Comparison_combo_cycles_YY_TTE_v4noSTOP.RData"),collapse=""))
    }
  }
}
